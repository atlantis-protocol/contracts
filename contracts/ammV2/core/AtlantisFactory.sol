...
import "./AtlantisPair.sol";

contract AtlantisFactory is IAtlantisFactory {
    ...
    modifier onlyOwner() {
        require(
            owner == msg.sender,
            "AtlantisFactory: caller is not the owner"
        );
        _;
    }
    ...
    function createPair(
        ...
        require(tokenA != tokenB, "AtlantisFactory: IDENTICAL_ADDRESSES");
        (address token0, address token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "AtlantisFactory: ZERO_ADDRESS");
        require(
            getPair[token0][token1] == address(0),
            "AtlantisFactory: PAIR_EXISTS"
        ); // single check is sufficient
        bytes memory bytecode = type(AtlantisPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        require(pair != address(0), "AtlantisFactory: FAILED");
        AtlantisPair(pair).initialize(token0, token1);
    ...
    function setOwner(address _owner) external onlyOwner {
        require(_owner != address(0), "AtlantisFactory: zero address");
    ...
    function setSetStableOwner(address _setStableOwner) external {
        require(
            ...
            "AtlantisFactory: not setStableOwner"
        );
        require(_setStableOwner != address(0), "AtlantisFactory: zero address");
    ...
    function setOwnerFeeShare(uint newOwnerFeeShare) external onlyOwner {
        require(
            newOwnerFeeShare > 0,
            "AtlantisFactory: ownerFeeShare mustn't exceed minimum"
        );
        require(
            newOwnerFeeShare <= OWNER_FEE_SHARE_MAX,
            "AtlantisFactory: ownerFeeShare mustn't exceed maximum"
        );
    ...
    function setReferrerFeeShare(
        ...
        require(referrer != address(0), "AtlantisFactory: zero address");
        require(
            referrerFeeShare <= REFERER_FEE_SHARE_MAX,
            "AtlantisFactory: referrerFeeShare mustn't exceed maximum"
        );
