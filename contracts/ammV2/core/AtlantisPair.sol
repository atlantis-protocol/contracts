...
import "./interfaces/IAtlantisPair.sol";
import "./UniswapV2ERC20.sol";
import "./libraries/Math.sol";
import "./interfaces/IERC20.sol";
import "./interfaces/IAtlantisFactory.sol";
import "./interfaces/IUniswapV2Callee.sol";

contract AtlantisPair is IAtlantisPair, UniswapV2ERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, "AtlantisPair: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function setFeePercent(
        ...
        require(
            msg.sender == IAtlantisFactory(factory).feePercentOwner(),
            "AtlantisPair: only factory's feeAmountOwner"
        );
        require(
            newToken0FeePercent <= MAX_FEE_PERCENT &&
                newToken1FeePercent <= MAX_FEE_PERCENT,
            "AtlantisPair: feePercent mustn't exceed the maximum"
        );
        require(
            newToken0FeePercent > 0 && newToken1FeePercent > 0,
            "AtlantisPair: feePercent mustn't exceed the minimum"
        );
    ...
    function setStableSwap(
        ...
        require(
            msg.sender == IAtlantisFactory(factory).setStableOwner(),
            "AtlantisPair: only factory's setStableOwner"
        );
        require(!pairTypeImmutable, "AtlantisPair: immutable");

        require(stable != stableSwap, "AtlantisPair: no update");
        require(
    ...
    function setPairTypeImmutable() external lock {
        require(
            msg.sender == IAtlantisFactory(factory).owner(),
            "AtlantisPair: only factory's owner"
        );
        require(!pairTypeImmutable, "AtlantisPair: already immutable");
    ...
    function _mintFee(
        ...
        (uint ownerFeeShare, address feeTo) = IAtlantisFactory(factory)
            .feeInfo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, "AtlantisPair: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
    ...
    function burn(
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "AtlantisPair: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function _swap(
        ...
        require(
            ...
            "AtlantisPair: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        ...
        require(
            ...
            "AtlantisPair: INSUFFICIENT_LIQUIDITY"
        );

        {
            require(
                to != tokensData.token0 && to != tokensData.token1,
                "AtlantisPair: INVALID_TO"
            );
        ...
        require(
            ...
            "AtlantisPair: INSUFFICIENT_INPUT_AMOUNT"
        );
...

        {
            ...
            uint referrerInputFeeShare = referrer != address(0)
                ? IAtlantisFactory(factory).referrersFeeShare(referrer)
                : 0;
    ...
    function sync() external lock {
        ...
        require(
            ...
            "AtlantisPair: liquidity ratio not initialized"
        );
