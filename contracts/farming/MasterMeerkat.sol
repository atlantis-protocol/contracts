...
contract Master is Ownable, ReentrancyGuard, Whitelist {
    ...
    constructor(
        ...
        totalAllocPoint = 0;
        whitelistAll = false;
        proxy = ProxyMeerkat(_proxyAddress);
    }
    ...
    function pendingMeerkat(
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 meerkatReward = multiplier
                .mul(meerkatPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accMeerkatPerShare = accMeerkatPerShare.add(
                meerkatReward.mul(1e18).div(lpSupply)
            );
        }
        return
            user.amount.mul(accMeerkatPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function depositNFT(
        ...
        require(
            ERC721(_nft).balanceOf(msg.sender) > 0,
    ...
    function deposit(
        ...
        uint256 _amount,
        address _referrer
    ) public nonReentrant nonContract {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (
            _amount > 0 &&
            address(meerkatReferral) != address(0) &&
            _referrer != address(0) &&
    ...
    function setMeerkatReferral(
        IMeerkatReferral _meerkatReferral
    ) public onlyOwner {
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (
            address(meerkatReferral) != address(0) && referralCommissionRate > 0
        ) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                ...
                meerkatReferral.recordReferralCommission(
                    referrer,
                    commissionAmount
                );
